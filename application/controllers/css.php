...
class Css extends CI_Controller {
	...
	public function staff_rendered(){
		if(isset($_SESSION['superuser'])){
			...
			$form = $this->database->cssRegional($year,$month);
	...
	public function personal_information(){
		foreach($this->database_pis->personal_information() as $row){
			echo $row['fname'].'<br>';
		}
	}
	...
	public function survey(){
		$personal_information = $this->database_pis->personal_information();
		$section=$this->database->section();
	...
	public function userlist(){
	    return $this->load->view('userlist');
	...
	public function reportsection(){
        return $this->load->view('reportsection');
	...
	public function generateweb(){
	    return $this->load->view('generateweb');
	...
	public function LoginAccess(){
		...
		if($row['username']==$uname and $row['userpassword']==$pword and $row['userstat']==1 and $row['usertype']==1){
			...
			$_SESSION['superuser']="IAN MANUGAS";
			$_SESSION['user']=null;
			redirect('css/admin');
		}
