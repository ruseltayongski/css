<?php
	...
	$approve=$this->database->userappr();
	...
	$year=$_SESSION['year'];
	$a=$this->database->cssList($office,$month,$year);
	foreach($a as $css)
	{
		$count++;
		if($css['cssstat']=="yes")
...
in_array($Etotalr6palld, $decimal) ? $Etmp6d = number_format($Etotalr6palld,0) : $Etmp6d = number_format($Etotalr6palld,2);
	if(isset($_POST['generate']))
	{
		...
		if($_POST['comment1'] != null or $_POST['comment2'] != null or $_POST['comment3'] != null)
		{
			$adminid=1;
			...
			$commentstat=1;
			$commentAdd=commentAdd($adminid,$comment1,$comment2,$comment3,$commentstat);
		}
...
<!DOCTYPE HTML>
	...
	<body>
		<div id="outer" style="background-color:white;color:black;">
           ...
           <center>
			<?php 
				echo "<h4><b>Monthly Report of Customer Feedback in<br>";
				echo $office."<br>".$month." ".$year."</b></h4>";
			?>
			</center>
		...
		<form action="" method="POST" name="form" id="form">
			...
			<div style = "display:inline">
				...
				<br>
				<textarea id = "analysis" placeholder="Type here..." name = "comment1" style="height:100px"></textarea>
			</div>
			<div style = "display:inline">
				<p id = "header"><b>IV. Recommendation:</b></p>
				<br>
				<textarea id = "recommendation" placeholder="Type here..." name = "comment2" style="height:100px"></textarea>
			</div>
				<p id = "header"><b>V. Action Taken/Action to Taken/Action:</b></p>
				<br>
				<textarea id = "action" placeholder="Type here..." name = "comment3" style="height:100px"></textarea>
				<br><br><br>
			<div>
			<table border="0">
				<tr>
					<td><b>Report Noted By:</b></td>
					<td>
						<select class="form-control" name="apprname" style = "border-radius:5px; width:200px;background-color:lightgreen;color:black;" required>
			               <option value="">Select Name</option>
			               <?php foreach($approve as $row)
			                {
			                if($_SESSION['userfname'] != $row['userfname'])
			                {
			                ?>
			                   <option value="<?php echo $row['userfname'];?>"><?php echo $row['userfname'].' '.$row['usermname'].' '.$row['userlname'];?></option>	                
			                <?php
			            	}
			                }
			                ?>
			...
			<tr><td><br>
				...
				<button class="btn btn-danger" type="submit" name = "generate" style="display:inline;">Generate Section(PDF)</button></td></tr>
			</table>
		</center>
		</form>
		</div>
